...
const RoomPage = ({ user, socket, users }) => {
  ...
  const handleClearCanvas = () => {
    ...
    ctx.fillRect = "white";
    ctx.clearRect(0, 0, canvasRef.current.width, canvasRef.current.height); //sets the pixels in a rectangular area to transparent black
    setElements([]);
  };
  ...
  return (
    <div className="row">
          ...
          <div className="w-100 mt-5 pt-5">
            {users.map((usr, index) => (
              <p key={index * 999} className="my-2 text-center w-100 ">
                {user.name} {user && user.userId === usr.userId && "(You)"}
              </p>
            ))}
          </div>
        </div>
      ...
      <div className="col-md-11 mx-auto mt-3 canvas-box ">
        <WhiteBoard
          canvasRef={canvasRef}
          ctxRef={ctxRef}
          elements={elements}
